...
library(shiny) ; library(flexdashboard) ; library(tidyverse) ; library(rtweet) ; library(tidytext) ; library(reshape2) ; library(igraph) ; library(grid) ; library(ggraph) ; library(lubridate) ; library(DT) ; library(htmltools) ; library(scales) ; library(emo) ; library(knitr) ; library(kableExtra) ; library(shinycssloaders) 
...
output$hashtags <- renderPlot({
  tweets() %>% 
    ...
    geom_point(colour = "#761B0C", size = 10) +
